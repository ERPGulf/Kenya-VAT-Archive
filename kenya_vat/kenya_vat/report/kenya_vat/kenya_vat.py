...
def get_columns():
	return [
		...
		},
		{
			...
			"label": _("Adjustment (KES)"),
			"fieldtype": "Currency",
			"width": 150,
		},
		{
			"fieldname": "vat_amount",
			"label": _("VAT Amount (KES)"),
			"fieldtype": "Currency",
			"width": 150,
		}
	]
...
def get_data(filters):
	...
	if frappe.db.exists('Kenya VAT Setting', company) is None:
		url = get_url_to_list('Kenya VAT Setting')
		frappe.msgprint(f'Create <a href="{url}">Kenya VAT Setting</a> for this company')
		return data

	kenya_vat_setting = frappe.get_doc('Kenya VAT Setting', company)
	...
	for vat_setting in kenya_vat_setting.kenya_vat_sales_accounts:
	...
	for vat_setting in kenya_vat_setting.kenya_vat_purchase_accounts:
		total_taxable_amount, total_taxable_adjustment_amount, \
...
def get_tax_data_for_each_vat_setting(vat_setting, filters, doctype):
	'''
	(Kenya, {filters}, 'Sales Invoice') => 500, 153, 10 \n
