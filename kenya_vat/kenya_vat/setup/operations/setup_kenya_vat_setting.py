...
def setup(company=None, patch=True):
	...
	make_custom_fields()
def add_permissions():
	"""Add Permissions for Kenya VAT Setting."""
	add_permission('Kenya VAT Setting', 'All', 0)
	for role in ('Accounts Manager', 'Accounts User', 'System Manager'):
		add_permission('Kenya VAT Setting', role, 0)
		update_permission_property('Kenya VAT Setting', role, 0, 'write', 1)
		update_permission_property('Kenya VAT Setting', role, 0, 'create', 1)

	"""Enable Kenya VAT Report"""
	frappe.db.set_value('Report', 'Kenya VAT', 'disabled', 0)
def create_kenya_vat_setting(self, method):
    """
    On creation of first company. Creates Kenya VAT Setting"""
    # Validating if this is the first company for Kenya
    company_list = frappe.get_all('Company', {
        'country': 'Kenya'
    }) 

    kenya_vat_setting = frappe.get_all('Kenya VAT Setting', {
        'company': self.name
    })

    if len(company_list) == 1 and len(kenya_vat_setting) == 0:
        make_custom_fields()
        file_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'kenya_vat_settings.json')
        with open(file_path, 'r') as json_file:
            account_data = json.load(json_file)
        

        # Creating Kenya VAT Setting
        kenya_vat_setting = frappe.get_doc({
            'doctype': 'Kenya VAT Setting',
            'company': self.name
        })
        ...
        for data in account_data:
            if data['type'] == 'Sales Account':
                for row in data['accounts']:
                    ...
                    account = row['account']
                    kenya_vat_setting.append('kenya_vat_sales_accounts', {
                        'title': row['title'],
                ...
                for row in data['accounts']:
                    item_tax_template = row['item_tax_template']
                    account = row['account']
                    kenya_vat_setting.append('kenya_vat_purchase_accounts', {
                        'title': row['title'],
                        'item_tax_template': f'{item_tax_template} - {self.abbr}',
                        'account': f'{account} - {self.abbr}'
                    })

        kenya_vat_setting.save()
...
def make_custom_fields(self, method):
	...
	sales_invoice_fields = [
			...
			dict(fieldname='kenya_einv_qr', label='Kenya E-Invoicing QR',
...
def update_regional_tax_settings(country, company):
	create_kenya_vat_setting(company)